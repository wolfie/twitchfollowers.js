...
/**
 * @param {number|TwitchCallback} offset
 * @param {number?} limit
...
function followers(offset, limit, callback) {
    ...
    limit = limit || followerHistoryCount;
...
function tick(previousFollowersCount) {
    ...
    timeoutId = setTimeout(function() {
        var offset = previousFollowersCount-followerHistoryCount;
        console.log("offset, followerHistoryCount: "+offset, followerHistoryCount);
        followers(offset, followerHistoryCount, callback);
    }, ticklength);
...
function start() {
    ...
    followers(function(followers){
        ...
        var f = followers.follows;
        for (var i=0; i<f.length/2; i++) {
            var temp = f[i];
            f[i] = f[f.length-i-1];
            f[f.length-i-1] = temp;
        }
        lastKnownFollowers = f;
        tick(initCount);
    });
}

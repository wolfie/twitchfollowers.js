...
sheet.insertRule('.cannon {' +
    ...
    'height: 20px;' +
    '}', 2);
sheet.insertRule('.cannon .base {' +
    'width: 20px;' +
    'height: 20px;' +
    'background-color: black;' +
    'position: absolute;' +
    '}', 3);
sheet.insertRule('.cannon .barrel {' +
    'width: 5px;' +
    'height: 30px;' +
    'background-color: red;' +
    'position: absolute;' +
...
function Cannon(x, y) {
    ...
    this.rotationRate = 0;
    this.rotation = 0;
    this.aimDegrees = 90 - (this.x / width - .5) * 90;

    this.sound_fire = new AudioEntity('sfx/cannon_fire.mp3');
    this.sound_impact = new AudioEntity('sfx/cannon_impact.wav');

    this.element_barrel = document.createElement('div');
    this.element_barrel.classList.add('barrel');
    this.element_barrel.style.transform = 'rotate('+((-1*this.aimDegrees)+90)+'deg)';
    this.element_base = document.createElement('div');
    this.element_base.classList.add('base');
}
...
Cannon.prototype.attach = function() {
    this.element.appendChild(this.element_barrel);
    this.element.appendChild(this.element_base);
...
Cannon.prototype.fire = function() {
    ...
    for (var i = 0; i < 50; i++) {
        ...
        confetto.setDirection(this.aimDegrees + r, .6 + Math.random() * .1);
